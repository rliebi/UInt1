...
public class CopyAddLoanView extends AbstractViewer {
	...
	public CopyAddLoanView(Library lib, Copy cop) {
		...
		pnlBookPart.setBorder(new TitledBorder(null, Messages
				.getString("MasterView.pnlBooks.title"), TitledBorder.LEADING,
				TitledBorder.TOP, null, null));
		contentPanel.add(pnlBookPart, BorderLayout.NORTH);
		JLabel lblSelectedBookTitleText = new JLabel(
				Messages.getString("CopyAddLoanView.lblSelectedBookTitleText"));
		JLabel lblSelectedBookCopyText = new JLabel(
				Messages.getString("CopyAddLoanView.lblSelectedBookCopyText"));

		String title = (copy.getTitle().getName().length() > 50) ? copy
				.getTitle().getName().substring(0, 50) : copy.getTitle()
				.getName();
		...
		gl_pnlBookPart
				.setHorizontalGroup(gl_pnlBookPart
		...
		pnlCustomersPart.setBorder(new TitledBorder(null, Messages
				.getString("MasterView.pnlCustomers.title"),
		...
		contentPanel.add(pnlSearch, BorderLayout.SOUTH);
		GridBagLayout gbl_pnlSearch = new GridBagLayout();
		gbl_pnlSearch.columnWeights = new double[] { 1.0 };

		pnlSearch.setLayout(gbl_pnlSearch);
		searchfield = new MySearchField(tblCustomers, 0, filters_customer);
		GridBagConstraints gbc_txtSearchfield = new GridBagConstraints();
		gbc_txtSearchfield.anchor = GridBagConstraints.WEST;
		pnlSearch.add(searchfield, gbc_txtSearchfield);
		...
		final JButton btnAddtoloan = new JButton(
				...
				new ListSelectionListener() {
					...
					public void valueChanged(ListSelectionEvent e) {
						...
						if (tblCustomers.getSelectedRowCount() == 1) {
							String s = tblCustomers.getValueAt(
									tblCustomers.getSelectedRow(), 0)
									.toString();
							if (s.contains(Messages
									.getString("CustomersInventoryView.Overdue")) == false
									&& Integer.parseInt(s.substring(0, 1)) < Setting
											.getMaxBorrowsPerCustomer()) {
		...
		tblCustomers.getColumnModel().getColumn(0)
