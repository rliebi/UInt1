...
public class ReturnLoanView extends AbstractViewer {
	...
	public ReturnLoanView(Library lib, final Loan l) {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(
				MasterView.class.getResource("/resources/images/logo_16.png")));
		...
		panel.setBorder(new TitledBorder(null, Messages
				.getString("LoanCloseView.loanClose.title"),
				TitledBorder.LEADING, TitledBorder.TOP, null, null));
		contentPane.add(panel, BorderLayout.CENTER);

		// Text
		JLabel lblBookTitelText = new JLabel(
				Messages.getString("Domain.Book.title"));
		JLabel lblCopyNumberText = new JLabel(
				Messages.getString("Domain.Copy.inventoryNumber"));
		JLabel lblReturnDateText = new JLabel(
				Messages.getString("Domain.Loan.returnDate"));
		JLabel lblBookConditionText = new JLabel(
				Messages.getString("Domain.Copy.condition"));

		// Value
		String LoanBookTitle = loan.getCopy().getTitle().getName();
		JLabel lblBooktitelvalue = new JLabel(
				(LoanBookTitle.length() > 80) ? LoanBookTitle.substring(0, 77)
						+ "..." : LoanBookTitle);
		JLabel lblCopynumbervalue = new JLabel(loan.getCopy()
				.getInventoryNumber() + "");
		...
		comboBoxBookCondition.setModel(new DefaultComboBoxModel<Condition>(
						...
						.addGroup(
								gl_panel.createParallelGroup(
										...
										.addComponent(lblBookTitelText,
												Alignment.LEADING,
												GroupLayout.DEFAULT_SIZE, 104,
												Short.MAX_VALUE))
						.addPreferredGap(ComponentPlacement.RELATED)
						.addGroup(
								gl_panel.createParallelGroup(Alignment.LEADING,
										false)
										.addComponent(comboBoxBookCondition, 0,
												GroupLayout.DEFAULT_SIZE,
												Short.MAX_VALUE)
						...
						gl_panel.createSequentialGroup()
								...
								.addGroup(
										gl_panel.createParallelGroup(
												Alignment.BASELINE)
												.addComponent(
														lblBookConditionText)
												.addComponent(
														comboBoxBookCondition,
														GroupLayout.PREFERRED_SIZE,
														GroupLayout.DEFAULT_SIZE,
														GroupLayout.PREFERRED_SIZE))
		...
		btnSave.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				if (txtReturndatevalue.validateDate()) {
					...
					if (!checkDateBeforePickup(loan, txtReturndatevalue)) {
						...
						loan.getCopy().setCondition(
								(Condition) comboBoxBookCondition
										.getSelectedItem());
						closeDialog();
					}
				}
			...
			private boolean checkDateBeforePickup(Loan selectedLoan,
